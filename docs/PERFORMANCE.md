...
- `npm run perf:lighthouse` runs three mobile Lighthouse CI lab measurements against the static production build and writes `.lighthouseci/`. On this Windows workstation the current local launch fails with Chrome Launcher `spawn UNKNOWN` before an audit; native Windows can also hit temporary-profile cleanup `EPERM`. The hosted Linux browser job is authoritative for LCP and CLS and for Firefox execution.
