...
- The local Lighthouse run remains partial because Windows Chrome profile cleanup returned `EPERM` after one sample. Hosted Linux CI run [36264020094](https://github.com/setnessconsulting/game-number-line-jumper/actions/runs/36264020094) passed all assertions and retained its three raw reports in `game-lighthouse-results-f7a269ccbbf754ac63e06aad585d18736fdc7ab2`. Median LCP was 1,359.31 ms and median CLS was 0. The run's build tree and all three payload file hashes match the frozen `main-0c87b2a` release artifact; its report-set hash and run/source identities are recorded in [`RELEASE_ACCEPTANCE.json`](./RELEASE_ACCEPTANCE.json).
